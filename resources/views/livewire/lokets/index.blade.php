<div class="space-y-6">
    <div class="flex flex-wrap items-end justify-between gap-4">
        <div class="space-y-2">
            <h1 class="text-xl font-semibold">Lokets</h1>
        ...
        <form wire:submit.prevent="create" class="flex items-end gap-2">
            <div>
                <label class="block text-xs font-medium">Nama Loket</label>
                <input type="text" wire:model.defer="nama_loket" class="rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900" required />
            </div>
            <div>
                <label class="block text-xs font-medium">Kode Prefix</label>
                <input type="text" wire:model.defer="kode_prefix" class="rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900" required />
            </div>
            <div>
                <label class="block text-xs font-medium">Deskripsi</label>
                <input type="text" wire:model.defer="deskripsi" class="rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900" />
            </div>
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-gray-50 dark:bg-gray-800">
                <tr>
                    ...
                    <th class="px-3 py-2 text-left">Nama Loket</th>
                    <th class="px-3 py-2 text-left">Kode Prefix</th>
            ...
            <tbody>
                @forelse($rows as $r)
                    <tr class="border-t border-gray-200 dark:border-gray-700">
                        ...
                        <td class="px-3 py-2">{{ $r['nama_loket'] ?? '' }}</td>
                        <td class="px-3 py-2">{{ $r['kode_prefix'] ?? '' }}</td>
                        <td class="px-3 py-2">{{ $r['deskripsi'] ?? '' }}</td>
                        <td class="px-3 py-2">
